...
export const useLcuStore = defineStore('lcu', () => {
  ...
  const filteredChampions = computed(() => {
    return champions.value.filter((c) => {
      ...
      // 价格范围过滤
      const price =
        filterCurrency.value === 'RP'
          ? (c.saleRpPrice ?? c.rpPrice)
          : (c.saleIpPrice ?? c.ipPrice)
      if (
        price !== null &&
        (price < priceRanges.value.min || price > priceRanges.value.max)
      ) {
        return false
      }
  ...
  async function checkStatus() {
    ...
    try {
      ...
      summoner.value = status.summoner ?? null
      wallet.value = status.wallet ?? null
      if (status.error) {
  ...
  async function loadChampions() {
    ...
    error.value = null
    try {
      champions.value = await window.api.getChampions()
    } catch (err) {
  ...
  async function purchaseSelected() {
    ...
    try {
      ...
      const currentWallet =
        currency === 'RP' ? (wallet.value?.rp ?? 0) : (wallet.value?.ip ?? 0)
